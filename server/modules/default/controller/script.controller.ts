...
export class Script {
  ...
  async get_device_list(@Query() query: any) {
    const scripts = await ScriptModel.getScriptAll(query.pageNum||1,query.pageSize||10);
    const count = await ScriptModel.count();
    return ResultUtils.success({ scripts:scripts,total:count});
  }
