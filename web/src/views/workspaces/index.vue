<template>
  <div class="dashboard-container">
        ...
        <el-col :span="12" style="height: 100%">
          <div class="checkbox-container">
            <div class="checkbox-header">
              <el-checkbox v-model="checkAllDevice" :indeterminate="isDevicesIndeterminate" @change="handleDeviceCheckAllChange">设备列表</el-checkbox>
              <el-tag size="mini" type="info" class="csp ml10" :effect="deviceFilter.state == 'online' ? 'dark': 'light'" @click="checkState('online')">在线({{ onlineDeviceCount }})</el-tag>
              <el-tag size="mini" type="info" class="csp ml2" :effect="deviceFilter.state == 'offline' ? 'dark': 'light'" @click="checkState('offline')">离线({{ offlineDeviceCount }})</el-tag>
              <el-tag v-for="category in $store.state.device.category" :key="category" type="info" :effect="checkedTag.includes(category) ? 'dark': 'light'" size="mini" class="csp ml2 mr5" @click="checkTag(category)">{{ category || '默认' }}</el-tag>
            </div>
            <div class="checkbox-group">
              <el-checkbox-group v-model="checkedDevices" @change="handleCheckedDeviceChange">
                <div v-for="item in devices" :key="item.device_id" class="checkbox-item">
                  <el-checkbox size="mini" :label="item.device_id">{{ item.name }} 【{{ item.category || '默认' }}】
                    <el-tag v-if='item.is_online' type="success" size="mini">在线</el-tag>
        ...
        <el-col :span="12" style="height: 100%">
          <div class="checkbox-container">
            ...
            <div v-loading="listLoading" class="checkbox-group">
              <div v-for="item in list" :key="item.script_id" class="checkbox-item">
                <el-radio v-model="checkedScript" :label="item.script_id">{{ item.script_name }}</el-radio>
              </div>
              <!--  <el-table highlight-current-row :show-header='false' @current-change="scriptSelected" size='mini' v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row>
            ...
            <div class="bottom">
              <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryForm.pageNum" :page-sizes="[10,20,30,50,60]" :page-size="queryForm.pageSize" layout="sizes,prev, pager, next" :total="total">
              </el-pagination>
            </div>
          </div>
        </el-col>
    ...
    <device-log class="device_log">
      <div>
        <el-button type="success" plain round size="mini" @click="stopAllScript">Stop All</el-button>
        <el-button type="primary" plain round size="mini" @click="runScript">Run Script</el-button>
      </div>
    </device-log>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    scriptSelected(val) {
      this.checkedScript = val.script_id;
    },
