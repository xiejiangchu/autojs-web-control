<template>
  <div class="app-container">
    <div class="mb15">
      <el-tag v-for="category in $store.state.device.category" :key="category" :type="checkedTag.includes(category) ? '' : 'info'" size="medium" class="mr10 csp" @click="checkTag(category)">{{ category || '默认' }}</el-tag>
    </div>
    <el-table size='mini' v-loading="listLoading" v-auto-height:maxHeight="-20" :max-height="maxHeight" :data="devices" element-loading-text="Loading" border fit highlight-current-row>
      ...
      <el-table-column label="类别" align="center">
        <template slot-scope="scope">
          <el-button type="text" @click="changeCategory(scope.row.device_id)">{{ scope.row.category || '默认' }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="是否在线" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.is_online" type="success">在线</el-tag>
